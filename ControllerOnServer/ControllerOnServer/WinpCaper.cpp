...
bool WinpCaper::isFromClient(const u_char* packet)
{
	for(int i=0;i<ETHER_ADDR_LEN;i++)
	{
...
struct Param
{
	u_char dstMACHost[ETHER_ADDR_LEN];
	CString filePath;
} threadParams;
void WinpCaper::sendConfigFile(const u_char dstMACHost[ETHER_ADDR_LEN],const CString& filePath)
{
	for(int i=0;i<ETHER_ADDR_LEN;i++)
	{
		threadParams.dstMACHost[i]=dstMACHost[i];
	}
...
{
	USES_CONVERSION;
	u_char* dstMACHost=threadParams.dstMACHost;
	...
	caper->sendPacket(dstMACHost,(u_char*)cmd,sizeof(Cmd_DeliverConfig));
	//发送文件体
	int len,i;
	char buff[1024];
	for(i=1;;i++)
