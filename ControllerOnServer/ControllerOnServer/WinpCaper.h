...
public:
	...
	bool isFromClient(const u_char* packet);
	void sendConfigFile(const u_char dstMACHost[ETHER_ADDR_LEN],const CString& filePath);
	friend UINT threadSendFile(LPVOID pParam);
};
