...
void WinpCapHelper::setMyHWnd(HWND hWnd,int i,int j)
{
	m_hWnd[i][j]=hWnd;
}
...
UINT catchPacket(LPVOID pParam)
{
	...
	while(caper->m_hWnd[HWND_Main]!=NULL)
	{
		...
		if(caper->isMyType(packetData)&&caper->isMyMAC(packetData))
		{
			...
			::SendMessage(caper->m_hWnd[etherHeader->ether_shost[5]-1][etherHeader->dstWnd],WM_USER_RECVFROMWINPCAP,NULL,(LPARAM)data);
		}
	}
...
void WinpCapHelper::sendConfigFile(unsigned long long dstAddrHex,const CString& filePath)
{
	threadParams.dstAddrHex=dstAddrHex;
	threadParams.filePath=filePath;
	AfxBeginThread(threadSendFile,this);
}
UINT threadSendFile(LPVOID pParam)
{
	USES_CONVERSION;
	unsigned long long dstAddrHex=threadParams.dstAddrHex;
	CString filePath=threadParams.filePath;
	WinpCapHelper* caper=(WinpCapHelper*)pParam;
	...
	caper->sendPacket(dstAddrHex,HWND_Main,(u_char*)cmd,sizeof(Cmd_DeliverConfig));
	//发送文件体
	int len,i;
	char buff[1024];
	for(i=1;;i++)
	{
		...
		if(len<1024)//发送文件结束
		{
			SetCmd_DeliverConfig(cmd,nodeID,SrcAddr_Outside,DstAddr_LocalDlg,i,len,1,(unsigned char*)buff);
			caper->sendPacket(dstAddrHex,HWND_Main,(u_char*)cmd,sizeof(Cmd_DeliverConfig));
			break;
		}
